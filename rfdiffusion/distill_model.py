...
class RFDiffusionDistiller:
    ...
    def take_step(self, model_type, x_t, timestep, seq=None, deterministic=True):
        ...
        score = compute_score_fn(x_t, timestep, seq=seq)
    ...
    def calculate_rama_score(self, backbone_coords, device=None):
        """
        Calculate Ramachandran plot validation score from backbone atom coordinates
        
        Args:
            backbone_coords: Tensor of shape [batch_size, protein_length, 14, 3] or the relevant
                            subset containing N, CA, C atoms
            device: Device to place tensors on (if None, uses backbone_coords's device)
        
        Returns:
            rama_scores: Tensor of shape [batch_size, protein_length-2] with Ramachandran scores
        ...
        if backbone_coords.device != device:
            backbone_coords = backbone_coords.to(device)
        
        # Extract N, CA, C atoms from the backbone coordinates
        # Assuming the atom order is [N, CA, C, ...] for each residue
        N = backbone_coords[:, :, 0, :]   # N atom coordinates
        CA = backbone_coords[:, :, 1, :]  # CA atom coordinates
        C = backbone_coords[:, :, 2, :]   # C atom coordinates
        ...
        # Tighter covariance matrices for better discrimination
        covs = torch.tensor([
            [0.09, 0.0, 0.0, 0.09],  # Alpha helix - tighter
            [0.09, 0.0, 0.0, 0.09],  # Beta sheet - tighter
            [0.03, 0.0, 0.0, 0.03]   # Left-handed helix - tighter
        ], device=device).view(3, 2, 2)
        
        weights = torch.tensor([0.6, 0.3, 0.1], device=device)
        ...
        # Add a stronger penalty for regions far from any allowed region with more gradual falloff
        min_mahalanobis, _ = torch.min(torch.stack(all_mahalanobis), dim=0)
        outlier_penalty = torch.exp(-0.05 * min_mahalanobis)  # More gradual falloff
        log_scores = log_scores - 0.5 * outlier_penalty  # Stronger penalty weight
        
        # Debug info to understand score distribution
    ...
    def calculate_cb_distances(self, backbone_coords, device=None):
        ...
        for b in range(batch_size):
            try:
                ...
                # Define ideal distance ranges for different sequence separations
                # These values are based on statistical analysis of protein structures
                ...
                # Calculate penalties for unrealistic distances
                local_penalty = self.distance_violation_score(cb_dists, local_mask, 
                                                        min_dist=4.0, max_dist=8.0, 
                                                        tight_min=5.0, tight_max=7.0)
                
                medium_penalty = self.distance_violation_score(cb_dists, medium_mask, 
                                                        min_dist=5.0, max_dist=14.0, 
                                                        tight_min=6.0, tight_max=12.0)
                ...
                distance_scores[b] = torch.tensor(0.0, device=device)
        
        return distance_scores

    def apply_softmin(self, scores, temperature=0.2):
        """
    ...
    def combined_structure_score(self, backbone_coords, device=None, weights=None):
        """
        Combined structure evaluation incorporating both Ramachandran validation
        and RFdiffusion-style inter-residue geometry
        ...
        # Apply softmin to focus on problematic regions
        rama_softmin = self.apply_softmin(rama_scores)
        
        # Calculate CB distance scores
        cb_dist_scores = self.calculate_cb_distances(backbone_coords, device)
        
        # Combine scores
        combined_score = weights['rama'] * rama_softmin + weights['cb_dist'] * cb_dist_scores.mean()
    ...
    def compute_structure_loss(self, pred, w_rama=0.2, w_cb=0.1, device=None):
        """
        Compute structure validation loss for predicted coordinates
        ...
        structure_score, components = self.combined_structure_score(
            pred, device, weights={'rama': w_rama, 'cb_dist': w_cb}
        )
        ...
        # Log components
        self._log.info(f"Structure validation: Rama={components['rama'].item():.4f}, CB={components['cb_dist'].item():.4f}, Loss={structure_loss.item():.4f}")
